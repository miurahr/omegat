        ...
        if (OT160RC9_VERSION.equals(CURRENT_VERSION)) {
            ...
            for (int i = 0; i < current.getMappingRules().size(); i++) {
                ...
                if (def.equals(maprule.getLanguageCode())) {
                    ...
                    maprule.getRules().removeAll(Objects.requireNonNull(getRulesForLanguage(defaults, LanguageCodes.ENGLISH_CODE)));
                    maprule.getRules().removeAll(Objects.requireNonNull(getRulesForLanguage(defaults, LanguageCodes.F_TEXT_CODE)));
                    maprule.getRules().removeAll(Objects.requireNonNull(getRulesForLanguage(defaults, LanguageCodes.F_HTML_CODE)));
                }
            }
        }
        return current;
